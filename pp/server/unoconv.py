    ...
    if "UNO_PATH" in os.environ:
        ...
        if os.name in ("nt", "os2"):
            if "PROGRAMFILES" in list(os.environ.keys()):
                extrapaths += glob.glob(os.environ["PROGRAMFILES"] +
                                        "\\LibreOffice*") + glob.glob(os.environ["PROGRAMFILES"] + "\\OpenOffice.org*")

            if "PROGRAMFILES(X86)" in list(os.environ.keys()):
                extrapaths += glob.glob(os.environ["PROGRAMFILES(X86)"] +
                                        "\\LibreOffice*") + glob.glob(os.environ["PROGRAMFILES(X86)"] +
                                                                      "\\OpenOffice.org*")
            ...
            extrapaths += (glob.glob("/usr/lib*/libreoffice*") + glob.glob("/usr/lib*/openoffice*") +
...
def office_environ(office):
    ...
    if os.name in ("nt", "os2"):
        ...
        if "LD_LIBRARY_PATH" in os.environ:
            os.environ["LD_LIBRARY_PATH"] = (office.unopath + os.pathsep + realpath(office.urepath, "lib") +
...
def python_switch(office):
    if office.pythonhome:
        ...
        os.environ["PYTHONPATH"] = (realpath(office.pythonhome, "lib") + os.pathsep +
                                    ...
                                    realpath(office.pythonhome, "lib", "site-packages") + os.pathsep + office.unopath)
...
class Options:
    ...
    def version(self):
        ...
        product = (uno.getComponentContext().ServiceManager.createInstance(
...
class Convertor:
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                if product.ooName not in ("LibreOffice",
                                          "LOdev") or LooseVersion(product.ooSetupVersion) <= LooseVersion("3.3"):
                ...
                info(
                    2,
                    "%s listener successfully started. (pid=%s)" % (product.ooName, ooproc.pid),
                )
                ...
                while timeout <= op.timeout:
                    ...
                    if retcode != None:
                        info(
                            3,
                            "Process %s (pid=%s) exited with %s." % (office.binary, ooproc.pid, retcode),
                        )
                        break
                    try:
                        unocontext = resolver.resolve("uno:%s" % op.connection)
    ...
    def getformat(self, inputfn):
        ...
        if not outputfmt:
            if doctype:
                print(
                    "unoconv: format [%s/%s] is not known to unoconv." % (op.doctype, op.format),
                    file=sys.stderr,
                )
            else:
                print(
    ...
    def convert(self, inputfn):
        ...
        try:
            ...
            # Cannot use UnoProps for FilterData property
            if op.importfilter:
                inputprops += (PropertyValue(
                    "FilterData",
                    0,
                    uno.Any("[]com.sun.star.beans.PropertyValue", tuple(op.importfilter)),
                    0,
                ),)
            ...
            if op.exportfilter:
                outputprops += (PropertyValue(
                    "FilterData",
                    0,
                    uno.Any("[]com.sun.star.beans.PropertyValue", tuple(op.exportfilter)),
                    0,
                ),)
            ...
            if hasattr(e, "ErrCode"):
                error(
                    "unoconv: UnoException during %s phase in %s (ErrCode %d)" % (phase, repr(e.__class__), e.ErrCode))
                exitcode = e.ErrCode
                pass
    ...
    if not op.listener and ooproc and convertor:
        ...
        # If there is no GUI attached to the instance, terminate instance
            ...
            try:
                ...
                info(
                    2,
                    "%s instance unsuccessfully closed, sending TERM signal." % product.ooName,
                )
                try:
                    ooproc.terminate()
                except AttributeError:
        ...
        if ooproc.poll() == None:
            ...
            info(
                2,
                "%s instance unsuccessfully terminated, sending KILL signal." % product.ooName,
            )
            try:
                ooproc.kill()
            except AttributeError:
            ...
            info(
                3,
                "Waiting for %s with pid %s to disappear." % (ooproc.pid, product.ooName),
            )
            ooproc.wait()
...
if __name__ == "__main__":
    ...
    info(
        3,
        "sysname=%s, platform=%s, python=%s, python-version=%s" % (os.name, sys.platform, sys.executable, sys.version),
    )
    ...
    for of in find_offices():
        ...
        try:
            ...
            #            debug_office()
            print(
                "unoconv: Cannot find a suitable pyuno library and python binary combination in %s" % of,
                file=sys.stderr,
            )
