...
def find_offices():
    ...
    for basepath in extrapaths:
        ...
        # MacOSX have soffice binaries installed in MacOS subdirectory, not
        # program
        unopath = "error"
        for basis in ("basis-link", "basis", ""):
...
def office_environ(office):
    ...
    os.environ["PATH"] = (
        realpath(office.basepath, "program") + os.pathsep + os.environ["PATH"]
    )

    # Set UNO_PATH so that "officehelper.bootstrap()" can find soffice
    # executable:
    ...
    if os.name in ("nt", "os2"):
        ...
        if "LD_LIBRARY_PATH" in os.environ:
            ...
            os.environ["LD_LIBRARY_PATH"] = (
                office.unopath + os.pathsep + realpath(office.urepath, "lib")
            )
...
class FmtList:
    ...
    def display(self, doctype):
        print(
            "The following list of %s formats are currently available:\n" % doctype,
            file=sys.stderr,
        )
        for fmt in self.list:
...
fmts.add(
    ...
    "stw",
    "stw",
    "Open Office.org 1.0 Text Document Template",
...
fmts.add(
    ...
    "sxw",
    "sxw",
    "Open Office.org 1.0 Text Document",
...
fmts.add(
    ...
    "etext",
    "txt",
    "Text Encoded (OpenOffice.org Writer/Web)",
...
fmts.add(
    ...
    "html10",
    "html",
    "OpenOffice.org 1.0 HTML Template",
    "writer_web_StarOffice_XML_Writer_Web_Template",
)  # 11
fmts.add("web", "html", "html", "HTML Document", "HTML")  # 2
fmts.add(
    "web", "html", "html", "HTML Document Template", "writerweb8_writer_template"
)  # 13
fmts.add("web", "mediawiki", "txt", "MediaWiki", "MediaWiki_Web")  # 9
fmts.add(
    "web", "pdf", "pdf", "PDF - Portable Document Format", "writer_web_pdf_Export"
)  # 10
fmts.add(
    "web",
    "sdw3",
    "sdw",
    "StarWriter 3.0 (OpenOffice.org Writer/Web)",
...
fmts.add(
    ...
    "sdw4",
    "sdw",
    "StarWriter 4.0 (OpenOffice.org Writer/Web)",
...
fmts.add(
    ...
    "sdw",
    "sdw",
    "StarWriter 5.0 (OpenOffice.org Writer/Web)",
...
fmts.add(
    ...
    "txt",
    "txt",
    "OpenOffice.org Text (OpenOffice.org Writer/Web)",
    "writerweb8_writer",
)  # 12
fmts.add(
    "web",
    "text10",
    "txt",
    "OpenOffice.org 1.0 Text Document (OpenOffice.org Writer/Web)",
    "writer_web_StarOffice_XML_Writer",
)  # 15
fmts.add(
    "web", "text", "txt", "Text (OpenOffice.org Writer/Web)", "Text (StarWriter/Web)"
)  # 8
fmts.add(
    "web",
    "vor4",
...
fmts.add(
    "spreadsheet", "ooxml", "xml", "Microsoft Excel 2003 XML", "MS Excel 2003 XML"
)  # 23
fmts.add(
    "spreadsheet", "ots", "ots", "ODF Spreadsheet Template", "calc8_template"
)  # 14
fmts.add(
    "spreadsheet", "pdf", "pdf", "Portable Document Format", "calc_pdf_Export"
)  # 34
...
fmts.add(
    "spreadsheet", "uos", "uos", "Unified Office Format spreadsheet", "UOF spreadsheet"
)  # 9
fmts.add(
    "spreadsheet",
    "vor3",
...
fmts.add(
    "graphics", "gif", "gif", "Graphics Interchange Format", "draw_gif_Export"
)  # 30
fmts.add(
    "graphics",
    "html",
...
fmts.add(
    "graphics", "jpg", "jpg", "Joint Photographic Experts Group", "draw_jpg_Export"
)  # 3
fmts.add("graphics", "met", "met", "OS/2 Metafile", "draw_met_Export")  # 43
fmts.add("graphics", "odd", "odd", "OpenDocument Drawing", "draw8")  # 6
fmts.add(
    "graphics", "otg", "otg", "OpenDocument Drawing Template", "draw8_template"
)  # 20
...
fmts.add(
    "graphics", "vor", "vor", "StarDraw 5.0 Template", "StarDraw 5.0 Vorlage"
)  # 36
fmts.add(
    "graphics", "vor3", "vor", "StarDraw 3.0 Template", "StarDraw 3.0 Vorlage"
)  # 35
...
fmts.add(
    "presentation", "gif", "gif", "Graphics Interchange Format", "impress_gif_Export"
)  # 18
fmts.add(
    "presentation",
    "html",
...
fmts.add(
    "presentation", "tiff", "tiff", "Tagged Image File Format", "impress_tif_Export"
)  # 12
fmts.add(
    "presentation",
    "uop",
...
fmts.add(
    "presentation", "vor", "vor", "StarImpress 5.0 Template", "StarImpress 5.0 Vorlage"
)  # 40
fmts.add(
    "presentation",
    "vor3",
    "vor",
    "StarDraw 3.0 Template (OpenOffice.org Impress)",
    "StarDraw 3.0 Vorlage (StarImpress)",
)  # 1
fmts.add(
    "presentation", "vor4", "vor", "StarImpress 4.0 Template", "StarImpress 4.0 Vorlage"
)  # 39
fmts.add(
    "presentation",
    "vor5",
    ...
    def __init__(self, args):
        ...
        for opt, arg in opts:
            if opt in ["-h", "--help"]:
                ...
                print(
                    "Warning: This option is deprecated by --output.", file=sys.stderr
                )
                self.output = arg
        ...
        if (
            not self.listener
            and not self.showlist
            and self.doctype != "list"
            and not self.filenames
        ):
            print(
                "unoconv: you have to provide a filename as argument", file=sys.stderr
            )
        ...
        if not self.connection:
            if not self.pipe:
                self.connection = (
                    "socket,host=%s,port=%s;urp;StarOffice.ComponentContext"
...
class Convertor:
    def __init__(self):
        ...
        try:
            ...
            if op.nolaunch:
                die(
                    113,
                    "Existing listener not found. Unable start listener by parameters. Aborting.",
                )

            # Start our own OpenOffice instance
            info(3, "Launching our own listener using %s." % office.binary)
            try:
                product = self.svcmgr.createInstance(
                    "com.sun.star.configuration.ConfigurationProvider"
                ).createInstanceWithArguments(
                    "com.sun.star.configuration.ConfigurationAccess",
                    UnoProps(nodepath="/org.openoffice.Setup/Product"),
                )
                if product.ooName not in ("LibreOffice", "LOdev") or LooseVersion(
                    product.ooSetupVersion
                ) <= LooseVersion("3.3"):
                    ooproc = subprocess.Popen(
                        [
                            office.binary,
                            "-headless",
                            "-invisible",
                ...
                info(
                    2,
                    "%s listener successfully started. (pid=%s)"
                    % (product.ooName, ooproc.pid),
                )

                # Try connection to it for op.timeout seconds (flakky
                # OpenOffice)
                timeout = 0
                while timeout <= op.timeout:
                    ...
                    error(
                        "Failed to connect to %s (pid=%s) in %d seconds.\n%s"
    ...
    def getformat(self, inputfn):
        ...
        # FIXME: This should go into the for-loop to match each individual
        # input filename
        if outputfmt:
        ...
        if not outputfmt:
            if doctype:
                print(
                    "unoconv: format [%s/%s] is not known to unoconv."
                    % (op.doctype, op.format),
                    file=sys.stderr,
                )
            else:
                print(
                    "unoconv: format [%s] is not known to unoconv." % op.format,
                    file=sys.stderr,
                )
            die(1)
    ...
    def convert(self, inputfn):
        ...
        try:
            ...
            # Load inputfile
            inputprops = UnoProps(
                Hidden=True, ReadOnly=True, UpdateDocMode=QUIET_UPDATE
            )
            ...
            try:
                ...
                raise UnoException(
                    ...
                    % (outputurl, e.ErrCode, outputprops),
                    None,
                )
            ...
            if hasattr(e, "Message"):
                ...
                error(
                    "unoconv: UnoException during %s phase in %s"
    ...
    if not op.listener and ooproc and convertor:
        ...
        # If there is no GUI attached to the instance, terminate instance
            ...
            try:
                ...
                info(
                    2,
                    "%s instance unsuccessfully closed, sending TERM signal."
                    % product.ooName,
                )
                try:
                    ooproc.terminate()
        ...
        if ooproc.poll() == None:
            ...
            info(
                2,
                "%s instance unsuccessfully terminated, sending KILL signal."
                % product.ooName,
            )
            try:
                ooproc.kill()
            ...
            info(
                3,
                "Waiting for %s with pid %s to disappear."
...
def main():
    ...
    try:
        ...
        error(
            "unoconv: could not find an existing connection to LibreOffice at %s:%s."
            % (op.server, op.port)
        )
        if op.connection:
            info(
                0,
                "Please start an LibreOffice instance on server '%s' by doing:\n\n    unoconv --listener --server %s --port %s\n\nor alternatively:\n\n    soffice -nologo -nodefault -accept=\"%s\""
            ...
            info(
                0,
                "Please start an soffice instance on server '%s' by doing:\n\n    soffice -nologo -nodefault -accept=\"socket,host=127.0.0.1,port=%s;urp;\""
                % (op.server, op.port),
            )
        exitcode = 1
...
if __name__ == "__main__":
    ...
    info(
        3,
        "sysname=%s, platform=%s, python=%s, python-version=%s"
    ...
    for of in find_offices():
        ...
        try:
            ...
            #            debug_office()
            print(
                "unoconv: Cannot find a suitable pyuno library and python binary combination in %s"
                % of,
                file=sys.stderr,
            )
